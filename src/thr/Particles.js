import particleImg from '../texture/particle1.jpg';
...
function Particles() {
	...
	this.init = function() {
		arr = new Array();
		texture = THREE.ImageUtils.loadTexture(particleImg);
	...
	this.join = function() {
		...
		for(var i = 0; i < 500; i++) {
			...
			//生成粒子的位置
			var __x = 400 - Math.random() * 800;
			var __y = 400 - Math.random() * 800;
			var __z = 400 - Math.random() * 800;

			//创建粒子的位置向量，并赋给它一些参数变量
			var temp = new THREE.Vector3(__x, __y, __z);
			temp.angle = Math.floor(Math.random() * 360); //粒子的初始角度，用它来计算粒子的初始位置
			temp.angleSpeed = 0.001+Math.random()*0.001; //粒子角度运动的速度
